<?php
/**
 * @version	$Id: api.blastchat.php 2013-01-30 15:24:18Z $
...
class BlastChatApi
{
	public function bc_createBlastChatData($bcData, $par) {
		...
		if ($bcData->detached) {
			echo "<script type=\"text/javascript\">bcData.detached = 1;</script>\n";
		}
	...
	protected function bc_getUserAvatar($user, $type) {
		...
		if ($type == "1") {
			if ($user->id) {
				global $user_email;
				get_currentuserinfo();
				$avt_file = md5( strtolower( trim ( $user_email ) ) );
				$avt_commonpath = "/avatar/";
			} else {
				//need guest avatar
			}
		}
